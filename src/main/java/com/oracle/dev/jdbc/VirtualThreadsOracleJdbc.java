...
public class VirtualThreadsOracleJdbc {
  ...
  public static void main(String args[]) throws SQLException {
    ...
    // virtual threads
    var threads = IntStream.range(0, 1_500)
  ...
  private static void doSQLWork(Connection conn, String queryStatement)
      throws SQLException {
    conn.setAutoCommit(false);
    try (Statement statement = conn.createStatement();
        ResultSet resultSet = statement.executeQuery(queryStatement)) {
      while (resultSet.next()) {
        System.out.println(new StringBuilder(resultSet.getString(1)).append(" ")
